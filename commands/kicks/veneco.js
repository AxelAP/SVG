...
module.exports = {
  ...
  async execute(client, message, args, discord) {
    ...
    let ausencia = "Murierón de hambre y.y";
    let reason = "Venezuela no se esta arreglando";
    let idUser = [autism[0], autism[3]];

    let random = Math.floor(Math.random() * idUser.length);
    let veneco = idUser.splice(random, 1);
    ...
    if (!autista)
      autista = await message.guild.members
        .fetch(idUser[0])
        .catch(console.error);
    ...
    let msg = `Oh no, **${autista.displayName}**, murio de hambre`;
